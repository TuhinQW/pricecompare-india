...
module.exports = async function handler(req, res) {
  ...
  if (!apiKey) {
    return json(res, 503, { ok: false, code: 'PROVIDER_NOT_CONFIGURED', error: 'Live marketplace provider is not configured yet.' });
  }
  ...
  if (!platforms.length) {
    return json(res, 400, { ok: false, error: 'No supported marketplace selected.' });
  }
